...
performTestcase( $log, $argv[1], $argv[2] );
return;
...
{
	global $conformance_base;
	$log->info(     "generate report for $testid in $conformance_base");
	$conformance_base = "D:/armed/XBLR/xbrlvalidation/demo/schemas";
	$log->info(     "generate report for $testid in $conformance_base");
	...
	$log->info(     "location of xml file $testCaseFolder");
	$log->info($testCase);
	...
	foreach ( $testCase->children()->variation as $key => /** @var SimpleXMLElement $variation */ $variation )
	{
		...
		foreach ( $instanceElements as $instanceElement )
		{
			...
			if ( $readMeFirst )
			{
				...
				$xbrlReportFolder = "D:/armed/XBLR/xbrlvalidation/demo/instances";
				$instanceFiles[ $name] = "$xbrlReportFolder/$instanceElement";
			}
		}
		...
		try
		{
			...
			else
			{
				...
				//$xbrlReportFolder = "D:/armed/XBLR/phpvalidation/demo/instances";
				...
				if ( $taxonomy->getHasFormulas() )
				{
					...
					{
						foreach ( $formulas->getVariableSets() as $variableSetQName => $variableSetForQName )
						{
							foreach ( $variableSetForQName as $variableSet )
							{
								//$result = $variableSet->compareResult( $testCaseFolder, $expectedResultNode );
								$log->info($variableSet->generatedUnsatisifiedMessages);
								foreach($variableSet->generatedUnsatisifiedMessages as $index => $msg){
									$log->info("UnsatisfiedMessage A - index : $index and msg : $msg");
								}
								foreach($variableSet->generatedSatisifiedMessages as $index => $msg){
									$log->info("SatisfiedMessage A index : $index and msg : $msg");
								}
					...
					{
						$json = array();
						$json['Messages'] = $formulas->getMsgResults(); 
						//var_dump($formulas->getMsgResults());
						...
						foreach ( $formulas->getVariableSets() as $variableSetQName => $variableSetForQName )
						{
							...
							foreach ( $variableSetForQName as $variableSet )
							{
								if(count($variableSet->generatedSatisifiedMessages) > 0){
									$json['SatisfiedMessages'][] = $variableSet->generatedSatisifiedMessages;
								}
								if(count($variableSet->generatedUnsatisifiedMessages) > 0){
									$json['UnsatisfiedMessages'][] = $variableSet->generatedUnsatisifiedMessages;
								}
								//var_dump($variableSet->generatedSatisifiedMessages);
								//var_dump($variableSet->generatedUnsatisifiedMessages);
								
								//$result = $variableSet->compareResult( $testCaseFolder, $expectedResultNode );
								// $log->info($variableSet->generatedUnsatisifiedMessages);
								// foreach($variableSet->generatedUnsatisifiedMessages as $index => $msg){
								// 	$log->info("UnsatisfiedMessage B - index : $index and msg : $msg");
								// }
								// foreach($variableSet->generatedSatisifiedMessages as $index => $msg){
								// 	$log->info("SatisfiedMessage Bindex : $index and msg : $msg");
								// }
								
							}
						}
						var_dump($json);
						$resultFile = fopen("D:/armed/XBLR/xbrlvalidation/demo/outputs/$testid.txt", "w") or die("Unable to open file!");
						fwrite($resultFile, json_encode($json, JSON_PRETTY_PRINT));
						fclose($resultFile);
